...
2. Follow `.cursor/rules/` **alwaysApply** rules identically to desktop — ship-loop tiers, repair loop, planning verify loop, **Fable adversarial pairing (D-22: Grok always challenges Fable deep spec/architecture)**, verification ladder, security gate, stall-advisor, and mandatory evidence lines (`model-gates.mdc` § Platform parity, D-20/D-21). No mobile-lite orchestration.
