...
import { assertVendorSessionForDelivery } from "./vendorSessionValidation";
import { hasAssignableSpot } from "./vendorDeliverySpotUtils";
import {
  computeCompleteAllItemTruth,
  computeExceptionItemTruth,
  itemTruthChanged,
  type LineExceptionInput,
  type ItemQtyTruth,
} from "./vendorDeliveredItemTruth";
...
export interface ApplyVendorDeliveredReceivingResult {
  deliveryId: string;
  status: DeliveryStatus;
  ...
  idempotent: boolean;
  itemsUpdated: number;
  readiness: Awaited<ReturnType<typeof applyDeliveryReadinessTransaction>>;
}
...
function resolveItemTruth(
  ...
  exceptionByItemId: Map<string, LineExceptionInput>,
): ItemQtyTruth {
  const qtyOrdered = Number(data.qtyOrdered ?? 0);
  if (!Number.isInteger(qtyOrdered) || qtyOrdered < 0) {
    throw new HttpsError("failed-precondition", "Invalid item ordered quantity.");
  }
  const exception = exceptionByItemId.get(itemId);
  if (exception) {
    try {
      return computeExceptionItemTruth(qtyOrdered, exception);
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Invalid line exception.";
      throw new HttpsError("invalid-argument", message);
    }
  }
  return computeCompleteAllItemTruth({
    qtyOrdered,
 ...
 */
export async function applyVendorDeliveredReceiving(
  db: admin.firestore.Firestore,
  input: {
    deliveryId: string;
    sessionToken: string;
    actorName: string;
    lineExceptions?: LineExceptionInput[];
  },
): Promise<ApplyVendorDeliveredReceivingResult> {
  ...
  if (!hasAssignableSpot(deliveryData)) {
  ...
  const exceptionByItemId = new Map<string, LineExceptionInput>();
  ...
  if (!receivingTruthEnabled && exceptionByItemId.size > 0) {
    throw new HttpsError(
      "failed-precondition",
      "Line exceptions require exception-only Delivered mode.",
    );
  }
  ...
  if (exceptionByItemId.size > 0) {
    ...
    for (const itemId of exceptionByItemId.keys()) {
      if (!itemIds.has(itemId)) {
        throw new HttpsError(
          "invalid-argument",
          "Exception item is not on this delivery.",
        );
      }
    }
  }
  ...
  const txResult = await db.runTransaction(async (tx) => {
    ...
    const freshData = freshSnap.data() as admin.firestore.DocumentData;
    if (!hasAssignableSpot(freshData)) {
      throw new HttpsError(
        "failed-precondition",
        "No assigned spot — ask dispatch.",
      );
    }
    const fresh = freshData as DeliveryDoc;
    ...
    const confirmedAt =
      alreadyConfirmed && fresh.vendorPhysicalDropoffConfirmedAt
        ? fresh.vendorPhysicalDropoffConfirmedAt
        : now;

    const skipQty =
      !receivingTruthEnabled ||
      (alreadyConfirmed && exceptionByItemId.size === 0);

    const plannedWrites: Array<{
      itemId: string;
      prior: admin.firestore.DocumentData;
      truth: ItemQtyTruth;
    }> = [];
    ...
    if (!skipQty) {
      for (const item of items) {
        ...
        if (!itemSnap.exists) {
          throw new HttpsError("failed-precondition", "Item missing.");
        }
        const prior = itemSnap.data() as admin.firestore.DocumentData;
    ...
    tx.update(deliveryRef, {
      ...
      deliveredAt:
        alreadyConfirmed && fresh.deliveredAt ? fresh.deliveredAt : now,
    ...
    for (const write of plannedWrites) {
      if (!itemTruthChanged(write.prior, write.truth)) continue;
      itemsUpdated += 1;
  ...
  const readiness = await applyDeliveryReadinessTransaction(db, deliveryId, {
