...
export async function loadEmailMatchContext(): Promise<MatchContext> {
  const db = getDb();
...
export async function loadExistingEmailIndex(): Promise<{
...
export function resolveTargetDeliveryId(
  match: { deliveryOrderId?: string; purchaseOrderId?: string },
  ctx: MatchContext,
): string | null {
  if (match.deliveryOrderId) return match.deliveryOrderId;
  if (!match.purchaseOrderId) return null;
  const candidates = ctx.deliveries.filter(
    (d) => d.purchaseOrderId === match.purchaseOrderId,
  );
