import type { OnboardingStatus, PhysicalLocation } from "./customerModels";

const FORWARD_CHAIN: OnboardingStatus[] = [
  "NEW",
  "CONFIGURING",
  "LAYOUT_DRAFT",
  "LAYOUT_APPROVED",
...
const ALLOWED: ReadonlyMap<OnboardingStatus, readonly OnboardingStatus[]> =
  new Map([
    ["NEW", ["CONFIGURING"]],
...
export function listAllowedOnboardingTransitions(
  from: OnboardingStatus,
): OnboardingStatus[] {
...
export function transitionOnboarding(
  ...
  if (!canTransitionOnboarding(from, to)) {
    throw new Error(
      `Illegal onboarding transition ${from} → ${to} for location ${location.locationId}`,
    );
  }

  const next: PhysicalLocation = {
    ...location,
    onboardingStatus: to,
    updatedAt: nowIso,
  };
...
export const ONBOARDING_FORWARD_ORDER = FORWARD_CHAIN;

const SETTLED_STATUSES: OnboardingStatus[] = [
  "ACTIVE",
  "CANCELED",
  "SUSPENDED",
  "PAST_DUE",
...
export function rollupCustomerOnboarding(
  locations: Pick<PhysicalLocation, "onboardingStatus">[],
): OnboardingStatus {
  if (locations.length === 0) return "NEW";

  const inProgress = locations.filter(
    (loc) => !SETTLED_STATUSES.includes(loc.onboardingStatus),
  );
  if (inProgress.length > 0) {
    let minIndex = Number.POSITIVE_INFINITY;
    let rollup: OnboardingStatus = "NEW";
