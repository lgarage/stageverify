...
exports.processInboundVendorEmail = void 0;
const admin = require("firebase-admin");
const https_1 = require("firebase-functions/v2/https");
const applyDeliveryReadiness_1 = require("./applyDeliveryReadiness");
const applyConflicts_1 = require("./email/applyConflicts");
const loadMatchContext_1 = require("./email/loadMatchContext");
const processEmailMessage_1 = require("./email/processEmailMessage");
...
function asInboundEmailMessage(data) {
    if (!data || typeof data !== "object") {
        throw new https_1.HttpsError("invalid-argument", "message payload is required.");
    }
    const raw = data;
    const sourceMessageId = typeof raw.sourceMessageId === "string" ? raw.sourceMessageId.trim() : "";
    const senderEmail = typeof raw.senderEmail === "string" ? raw.senderEmail.trim().toLowerCase() : "";
    ...
    if (!sourceMessageId ||
        ...
        !receivedAt) {
        throw new https_1.HttpsError("invalid-argument", "Invalid inbound email message.");
    }
    ...
    return {
        sourceMessageId,
        threadId: typeof raw.threadId === "string" && raw.threadId.length <= MAX_MESSAGE_ID_LEN
            ? raw.threadId.trim()
            : undefined,
        senderEmail,
        recipientEmails,
 ...
 */
exports.processInboundVendorEmail = (0, https_1.onCall)({
    region: "us-central1",
    cors: [
        "http://localhost:5173",
        "http://127.0.0.1:5173",
    ...
    if (!request.auth?.uid) {
        throw new https_1.HttpsError("permission-denied", "Dispatcher sign-in required for email ingestion.");
    }
    const message = asInboundEmailMessage(request.data?.message);
    ...
    const existing = await (0, loadMatchContext_1.loadExistingEmailIndex)();
    const result = (0, processEmailMessage_1.processInboundEmail)(message, ctx, existing);
    ...
    const baseEvent = {
        ...
        reviewStatus: result.reviewStatus === "auto_processed"
            ? "auto_processed"
            : result.duplicate
                ? "rejected"
                : "pending_review",
    ...
    const deliveryOrderId = (0, loadMatchContext_1.resolveTargetDeliveryId)(result.match, ctx);
    if (!deliveryOrderId) {
        const pendingEvent = {
            ...baseEvent,
    ...
    if (!deliverySnap.exists) {
        throw new https_1.HttpsError("not-found", "Matched delivery not found.");
    }
    const deliveryData = deliverySnap.data();
    if (deliveryData.vendorOrderComplete === true &&
    ...
    const itemsSnap = await getDb()
        ...
        .get();
    if (itemsSnap.empty) {
        throw new https_1.HttpsError("failed-precondition", "Delivery has no line items.");
    }
    if (itemsSnap.size > 500) {
        throw new https_1.HttpsError("failed-precondition", "Delivery has too many line items for email apply.");
    }
    const items = itemsSnap.docs.map((doc) => doc.data());
    const conflictReason = (0, applyConflicts_1.hasVendorOrderCompleteApplyConflict)({
        vendorPhysicalDropoffConfirmed: deliveryData.vendorPhysicalDropoffConfirmed === true,
        vendorOrderComplete: deliveryData.vendorOrderComplete === true,
        vendorOrderCompleteSource: typeof deliveryData.vendorOrderCompleteSource === "string"
    ...
    const patch = {
        ...(0, processEmailMessage_1.buildVendorOrderCompletePatch)(now),
    ...
    await getDb().runTransaction(async (tx) => {
        ...
        if (fresh.vendorOrderComplete === true &&
            fresh.vendorOrderCompleteSource === "vendor_email") {
            throw new https_1.HttpsError("already-exists", "Vendor order complete already applied from email.");
        }
        const freshConflict = (0, applyConflicts_1.hasVendorOrderCompleteApplyConflict)({
            vendorPhysicalDropoffConfirmed: fresh.vendorPhysicalDropoffConfirmed === true,
            vendorOrderComplete: fresh.vendorOrderComplete === true,
            vendorOrderCompleteSource: typeof fresh.vendorOrderCompleteSource === "string"
...
//# sourceMappingURL=processInboundVendorEmail.js.map