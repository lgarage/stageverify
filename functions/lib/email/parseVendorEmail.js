...
exports.parseVendorEmail = parseVendorEmail;
...
function uniqueMatches(pattern, text, prefix) {
    const found = new Set();
    for (const match of text.matchAll(pattern)) {
        const num = match[1];
        if (num)
            found.add(`${prefix}${num.replace(/\s/g, "")}`);
    }
    return [...found];
}
...
function parseVendorEmail(message) {
    ...
    const vendorOrderCompleteClaim = classification === "vendor_order_complete" ||
        classification === "remaining_items_delivered";
    let estimatedDeliveryDate;
    const dateMatch = combined.match(/(?:delivery|eta|arrive)[^\n]{0,40}(\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}\/\d{2,4})/i);
    if (dateMatch?.[1])
        estimatedDeliveryDate = dateMatch[1];
    return {
        classification,
        poNumbers,
        orderNumbers,
...
function normalizeSubjectForFingerprint(subject) {
    return subject
...
//# sourceMappingURL=parseVendorEmail.js.map