...
exports.parseFirstSupplyInvoicePage = parseFirstSupplyInvoicePage;
...
function dedupeRepeatedName(line) {
...
function inferFirstSupplyFulfillment(shipViaRaw, text) {
    const shipVia = shipViaRaw?.trim() ?? "";
    if (/\b(?:COUNTER|EXPRESS)\s+PU\b/i.test(shipVia))
        return "will_call_pickup";
    if (/\bPU\b/i.test(shipVia))
        return "will_call_pickup";
    return (0, inferImportStatus_1.inferFulfillmentMethod)("", shipViaRaw, text);
}
function parseFirstSupplyLines(text) {
    const tableStart = text.search(/Ln\s*#\s*\nDescription Ordered/i);
    const tableEnd = text.search(/\d+\s+Lines Total/i);
    const tableText = tableStart >= 0 ? text.slice(tableStart, tableEnd >= 0 ? tableEnd : undefined) : text;
    const rows = tableText.split("\n");
    const lines = [];
    let i = 0;
    while (i < rows.length) {
        const row = rows[i]?.trim() ?? "";
        const match = row.match(/^(\d+)\s+(\S+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+(\S+)\s+([\d.]+)\s+([\d.]+)$/);
        if (match) {
...
function pageTextFingerprint(page) {
    ...
    if (invoice)
        return `first-supply:${invoice}`;
    return `first-supply:${page.pageId}`;
}
function parseFirstSupplyInvoicePage(page) {
    ...
    const invoiceDate = normalizeShortDate(capture(/Invoiced\s+(\d{2}\/\d{2}\/\d{2})/i, text) ?? orderDate);
    ...
    return {
        header: {
            ...
            fulfillmentMethod,
            shipCompletePolicy: (0, inferImportStatus_1.inferShipCompletePolicy)(text),
            shipViaRaw,
        },
        lines,
        orderNotes: [],
...
//# sourceMappingURL=parseFirstSupplyInvoice.js.map