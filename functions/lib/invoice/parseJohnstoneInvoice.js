...
function captureLabeledField(label, valuePattern, text) {
    const escaped = label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const re = new RegExp(`${escaped}(?:\\s*#)?(?:\\s*:\\s*|\\s+)(${valuePattern})`, "i");
...
function parseShipViaToken(raw) {
    const trimmed = raw.trim();
    if (!trimmed)
        return undefined;
...
function parseWideRowShipVia(middle) {
    const truck = middle.match(/\s+(TRUCK\s+DELIVE\w*)\s*$/i);
    if (truck)
        return parseShipViaToken(truck[1]);
    const pickup = middle.match(/\s+(PICKUP)\s*$/i);
    if (pickup)
        return parseShipViaToken(pickup[1]);
    const willCall = middle.match(/\s+(WILL\s*[- ]?\s*CALL\b.*)\s*$/i);
    if (willCall)
        return parseShipViaToken(willCall[1]);
...
function parseTabularHeaderBlock(text) {
    const partial = {};
    ...
    for (let i = 0; i < lines.length - 1; i += 1) {
        ...
        if (/^Customer\s*#\s+Sales\s+Order\s*#\s+Customer\s+P\/O/i.test(labelLine)) {
            const withDate = valueLine.match(/^(\d{3,10})\s+(\d{3,10})\s+(.+?)\s+(\d{1,2}\/\d{1,2}\/\d{2,4})\s*$/);
            if (withDate) {
        ...
        if (/^Buyer\s+Ship\s+Via/i.test(labelLine)) {
            const shipViaMatch = valueLine.match(/^(.+?)\s+(TRUCK\s+DELIVE\w*|WILL\s*[- ]?\s*CALL\b.*)$/i);
            if (shipViaMatch) {
        ...
        if (/^Order\s+Date\s+Buyer/i.test(labelLine)) {
            const orderBuyer = valueLine.match(/^(\d{1,2}\/\d{1,2}\/\d{2,4})\s+(.+)$/);
            if (orderBuyer) {
        ...
        if (/^Customer\s*#\s+Order\s+Date\s+Sales\s+Order\s*#\s+Buyer\s+Customer\s+P\/O\s*#\s+Ship\s+Via/i.test(labelLine)) {
            ...
            if (wide) {
                ...
                if (shipVia) {
                    ...
                    const shipViaTail = middle.match(new RegExp(`\\s+${shipVia.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*$`));
...
function isInvoiceFooterLine(trimmed) {
    return (INVOICE_FOOTER_INLINE.test(trimmed) ||
        /^(?:Remit\s+To|Taxable\b|POS\s+Copy|Merchandise\b|Freight\b|Sub\s+Total|TOTAL\b|Suspended|Equipment|of damage|Misc Charges)/i.test(trimmed));
}
function truncateAtInvoiceFooter(text) {
...
function pushParsedLine(lines, parsed) {
    ...
    const { lineType, excludeFromExpectedItems } = classifyLine(parsed.vendorProductNumber, parsed.quantityShipped, description);
    lines.push({
        ...parsed,
        description,
        filteredNotes: [],
...
function parseLineTableRows(lineSection, orderNotes) {
    ...
    for (const rawLine of remainder.split("\n")) {
        ...
        if (/REPAIR/i.test(trimmed)) {
            ...
            prev.description = truncateAtInvoiceFooter([prev.description, trimmed].filter(Boolean).join(" ").trim());
        }
    }
    return lines;
}
