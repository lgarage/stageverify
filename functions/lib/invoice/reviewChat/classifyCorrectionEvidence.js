...
exports.classifyCorrectionEvidence = classifyCorrectionEvidence;
/**
...
function hasBoundedToken(haystack, needle) {
...
const POSITIVE_ASSERTION_RE = /\b((use|set|update|change|correct|fix|capture|apply)\b[\s\S]{0,80}?\b(to|as|=|:)|(use|set|update|capture)\b)\s*[“"']?/i;
const NEGATION_RE = /\b(isn'?t|is not|not|don'?t|do not|never|wrong|incorrect|missing)\b/i;
function findExactDispatcherAssertion(recentDispatcherTexts, proposedValue) {
    ...
    for (const text of recentDispatcherTexts) {
        ...
        if (NEGATION_RE.test(raw))
            continue;
        if (!POSITIVE_ASSERTION_RE.test(raw))
            continue;
        if (!hasBoundedToken(t, proposedValue))
            continue;
        const longerToken = t
            .split(/[^A-Z0-9]+/)
...
function classifyCorrectionEvidence(input) {
    const proposedValue = input.proposedValue.trim();
    if (!proposedValue || proposedValue.length < MIN_PROPOSED_VALUE_CHARS) {
        return { sourceType: null };
    }
    const span = (0, reviewAgentContext_1.findEvidenceSpan)(input.combinedExtractedText, proposedValue);
    if (span) {
        ...
        if (matchedNorm === proposedNorm) {
            const leftChar = span.start > 0 ? input.combinedExtractedText[span.start - 1] : "";
            const rightChar = span.end < input.combinedExtractedText.length
                ? input.combinedExtractedText[span.end]
                : "";
            const leftBoundary = span.start === 0 || /[^A-Za-z0-9]/.test(leftChar);
            const rightBoundary = span.end >= input.combinedExtractedText.length ||
    ...
    if (hasBoundedToken(input.combinedExtractedText, proposedValue)) {
        const span2 = (0, reviewAgentContext_1.findEvidenceSpan)(input.combinedExtractedText, proposedValue);
        if (span2) {
            const leftChar = span2.start > 0 ? input.combinedExtractedText[span2.start - 1] : "";
            const rightChar = span2.end < input.combinedExtractedText.length
                ? input.combinedExtractedText[span2.end]
                : "";
            const leftBoundary = span2.start === 0 || /[^A-Za-z0-9]/.test(leftChar);
            const rightBoundary = span2.end >= input.combinedExtractedText.length ||
                /[^A-Za-z0-9]/.test(rightChar);
            if (leftBoundary && rightBoundary) {
                return {
                    sourceType: "document_evidence",
...
//# sourceMappingURL=classifyCorrectionEvidence.js.map