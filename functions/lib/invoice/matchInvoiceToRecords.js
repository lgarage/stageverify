...
function addCandidate(map, delivery, scoreDelta, reason) {
 ...
 */
function matchInvoiceToRecords(vendorInvoiceImportId, header, ctx, deliveryNotesById) {
    const reasons = [];
    let purchaseOrderId;
    let jobId;
    let vendorId;
    ...
    if (poHint) {
        const exactPo = ctx.purchaseOrders.filter((po) => po.poNumber.toUpperCase() === poHint.toUpperCase());
        if (exactPo.length === 1) {
        ...
        const poByReference = ctx.purchaseOrders.filter((po) => po.poNumber.toUpperCase() === customerPo.toUpperCase());
    ...
    for (const key of orderKeys) {
        const orderMatches = ctx.deliveries.filter((d) => d.orderNumber.toUpperCase() === key.toUpperCase());
    ...
    if (jobHint) {
        const jobMatches = ctx.jobs.filter((j) => j.jobNumber.toUpperCase() === jobHint.toUpperCase());
...
//# sourceMappingURL=matchInvoiceToRecords.js.map