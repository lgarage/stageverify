...
exports.sanitizeVendorInvoiceImportForClient = sanitizeVendorInvoiceImportForClient;
...
function trimHeaderValue(value) {
    ...
    if (trimmed.length <= MAX_HEADER_STRING)
        return trimmed;
    return `${trimmed.slice(0, MAX_HEADER_STRING)}…`;
}
function sanitizeVendorInvoiceImportForClient(doc) {
    const header = {};
    ...
    const parsedLines = (doc.parsedLines ?? []).map((line) => ({
        ...line,
        description: line.description.length > 240
...
//# sourceMappingURL=sanitizeVendorInvoiceImport.js.map