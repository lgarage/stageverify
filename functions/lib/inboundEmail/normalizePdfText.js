...
exports.hasCustomFontPdfEncoding = hasCustomFontPdfEncoding;
exports.normalizeCustomFontPdfText = normalizeCustomFontPdfText;
exports.adaptJohnstoneMultiColumnLayout = adaptJohnstoneMultiColumnLayout;
exports.canonicalizeJohnstoneLineGrid = canonicalizeJohnstoneLineGrid;
exports.postProcessExtractedPdfText = postProcessExtractedPdfText;
/** True when text likely needs U+XX00 normalization (low ASCII ratio + BMP high-byte glyphs). */
function hasCustomFontPdfEncoding(text) {
...
function normalizeCustomFontPdfText(raw) {
    const mapped = [...raw]
        .map((c) => {
        const cp = c.codePointAt(0) ?? 0;
        if (cp === 0x1100)
 ...
 */
function adaptJohnstoneMultiColumnLayout(text) {
    const lines = text.replace(/\r\n/g, "\n").split("\n").map((l) => l.trim()).filter(Boolean);
    for (let i = 0; i < lines.length - 1; i += 1) {
        const label = lines[i];
        const value = lines[i + 1];
        if (!/Customer\s*#/i.test(label) || !/Sales\s+Order\s*#/i.test(label))
            continue;
        if (!/Customer\s+P\/O/i.test(label))
            continue;
        const m = value.match(/^(\d{3,10})\s+(\d{1,2}\/\d{1,2}\/\d{2,4})\s+(\d{3,10})\s+([A-Z][A-Z]+(?:\s+[A-Z][A-Z]+)?)\s+(.+)$/i);
        if (!m)
            continue;
...
function canonicalizeJohnstoneLineGrid(text) {
    let out = text.replace(/LN\s+QNTY\s+QNTY\s+QNTY/gi, "LN QNTY ORD QNTY SHIP QNTY B/O");
    out = out.replace(/LN\s+QNTY\s+QNT\s+QNT/gi, "LN QNTY ORD QNTY SHIP QNTY B/O");
    // Some invoices omit B/O=0 before product (e.g. B72-487, G80-540) — inject fourth qty column.
    out = out.replace(/^(\d+)\s+(\d+)\s+(\d+)\s+([A-Z][\w-])/gm, "$1 $2 $3 0 $4");
    return out;
}
/** Full post-extract pipeline: custom-font normalize + layout adapter. */
function postProcessExtractedPdfText(raw) {
...
//# sourceMappingURL=normalizePdfText.js.map