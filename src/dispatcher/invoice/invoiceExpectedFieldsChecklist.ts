...
function buildLineRows(importRow: VendorInvoiceImportReview): ExpectedFieldRow[] {
  ...
  if (lineCount === 0) {
    let notes =
      "No lines stored. If the PDF had a line table, use Refresh in the review modal (or Refresh Now for full sync).";
    if (orderNum === "4046362") {
      notes =
...
export function buildExpectedJohnstoneFieldChecklist(
  ...
  const zeroLinesNote =
    lineCount === 0
      ? "Zero parsed lines — use Refresh above if the PDF has line items (or Refresh Now for a full Gmail sync)."
      : undefined;
