...
function captureHeaderInvoiceNumber(text: string): string | undefined {
  const labeled = capture(
    /Invoice\s*#\s+(?!Invoice(?:\s+Date|\s*#)|Date\b)([A-Z0-9]*\d[A-Z0-9-]*)/i,
    text,
  );
...
function isPlausiblePoValue(raw: string | undefined): boolean {
  ...
  if (/\bShip\s+Via\b/i.test(value) && !/\bRETURN\b/i.test(value)) return false;
  return true;
}
...
function pickPoValue(...values: Array<string | undefined>): string {
  for (const value of values) {
    ...
    if (isPlausiblePoValue(raw)) return raw;
  }
  return "";
}
  ...
  for (let i = 0; i < lines.length - 1; i += 1) {
    ...
    if (/^Invoice\s*#\s+Invoice\s+Date/i.test(labelLine)) {
      const invRow = valueLine.match(
        /^([A-Z0-9-]*\d[A-Z0-9-]*)\s+(\d{1,2}\/\d{1,2}\/\d{2,4})(?:\s+(\d{1,2}\/\d{1,2}\/\d{2,4}))?/,
      );
      if (invRow) {
        partial.vendorInvoiceNumber = invRow[1];
        partial.invoiceDate = invRow[2];
        if (invRow[3]) partial.shipDate = invRow[3];
      }
    }

    if (
      /^Customer\s*#\s+Order\s+Date\s+Sales\s+Order\s*#\s+Buyer\s+Customer\s+P\/O\s*#\s+Ship\s+Via(?:\s+Salesman)?/i.test(
        labelLine,
      )
    ) {
...
function isStackedLabelNoise(value: string | undefined): boolean {
  const trimmed = (value ?? "").trim();
  if (!trimmed) return true;
  return /^(?:Order\s+Date|Sales\s+Order|Customer\s+P\/?O|Ship\s+Via|Buyer|Invoice\s*#|Invoice\s+Date|Ship\s+Date|Freight|Terms|Job\s+Number|#)/i.test(
    trimmed,
  );
}
...
const LINE_ROW_CREDIT =
  /^(\d+)\s+(-?\d+)\s+(-?\d+)\s+([A-Z0-9-]+)\s+(\S+)\s+(.+?)\s+[\d.]+\s+[\d.]+\s+(\$[\d,.-]+)/i;
...
function isDescriptionContinuation(trimmed: string): boolean {
  ...
  if (/^If you have any questions/i.test(trimmed)) return false;
  if (/^Merchandise\b/i.test(trimmed)) return false;
...
const REMIT_TO_LINE_BLEED =
  /^(?:Taxable|Merchandise|Freight|Sub\s+Total|TOTAL|Misc\s+Charges)\b[\s\d.$-]*/i;

function stripRemitToLineBleed(line: string): string {
...
function parseRemitToBlock(text: string): {
  ...
  for (const line of blockLines.slice(1)) {
    if (/^please call/i.test(line)) break;
    if (REMIT_TO_LINE_BLEED.test(line)) continue;
    if (/^(?:POS\s+Copy|Suspended|Equipment|of damage)/i.test(line)) break;
    addressLines.push(line);
    if (/\b(?:SD|WI|MN|IA)\s+\d{5}\b/i.test(line)) break;
  }
...
function splitDuplicatedWideNames(line: string): { soldToName: string; shipToName: string } {
...
function parseSoldShipToBlock(text: string): {
  soldToName: string;
  shipToName: string;
  ...
  const addrMatch = afterNames.match(
    /^\s*\n\s*(\d{2,}[^\n]+(?:WI|MN|IA|SD)\s+\d{5}[^\n]*)/im,
  );
