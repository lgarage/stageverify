...
**Shipped (2026-08-20):** Scoped deploy **`getJobVendorDeliveries` only** — hash `7f1d3d2959fc7e3d26528e35c010a02bc175b63e` gen `1787191007713177`. Job-PIN prod 22/22 (real job name + ORD-005 + PO-88390 + collapsed DELIVERED); all-vendor prod 3/3. No other CFs / no rules / no indexes.

**Shipped (2026-08-20):** All-vendor compact delivery cards (job + order/invoice + PO on the face; company-run starts collapsed; Settings Archived Users) — **v0.0.295** gh-pages LIVE (`index-CVmqccep.js` / Pages tip `46b149e`). Job-scoped compact fields now served by the ACTIVE `getJobVendorDeliveries` revision above.
