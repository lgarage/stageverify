...
**Shipped (2026-08-28):** Invoice Review **Cancel + confirm gates + phone card boundaries** (**v0.0.360**) — footer Cancel exits inspect with no write; Approve/Reject require Confirm before existing wizard/reason-dialog; collapsed phone cards have stronger border/radius/gap. Desktop rows unchanged. Isolated from resent-invoice idempotency (PR #238). FE-only. **WAITING ON DAN REAL-IPHONE INVOICE-REVIEW UX TEST.**

**Shipped (2026-08-28):** Internal mobile **drawer + header + filters** (**v0.0.359**) — narrower hamburger drawer (~72% vw), compact 2×2 topbar actions, 2-col status filter grid; collapse accordion from v0.0.358 preserved. FE-only; no Google auth / PIN / CF. **READY FOR DAN REAL-IPHONE DRAWER + HEADER + FILTER TEST.**
...
**Shipped (2026-08-28):** vendor login CF body opt LIVE (PR #232 / `a08cadf7`) on `resolveLocationScanPin` + `getVendorRunDeliveries`. Unused T0→T4 1240ms then 4930ms; warm 1520ms. List 220–300ms when up; PIN unused still 1.0–4.7s. P0 not resolved.
...
**Shipped (2026-08-28):** first unused login scans pre-351 `stageverify_vendor_run_list_*` cache — **v0.0.352** gh-pages FE-only; no `minInstances`. Real iPhone idle &lt;2s still requires Dan acceptance.
...
**Shipped (2026-08-28):** stop `#/s` CF keep-alive at PIN Verify — **v0.0.347** gh-pages FE-only; no `minInstances`. Real iPhone idle &lt;2s still requires Dan acceptance.
...
**Shipped (2026-08-31):** Vendor Complete tap repair — **v0.0.365** FE-only; `vendorRunCanComplete` + action-row hit-target; no CF/rules. Await Dan iPhone retest.
**Shipped (2026-08-29):** Vendor Complete delivery confirm + keep completed row visible — **v0.0.364** FE-only; confirm before `markVendorDeliveriesBulk`; FE merge restores CF-omitted `<72h` completed rows; no CF/rules.
...
- **View Full Email Chain modal (v0.0.345):** ✅ LIVE — Delivery Details **View Full Email Chain** opens a large centered read-only overlay (Vendor Communications size); drawer stays open; same chain data/order as Review Vendor Email; no in-drawer dump
