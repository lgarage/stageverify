...
- **Dispatcher Deliveries column order (2026-08-08):** 🔵 PR — locked Status → Fulfillment → Vendor → Job Name → Invoice # → PO # → Staging Location → Items → Delivery / Pickup Date → Issue → Assigned Technician → Action; Job #/Order # table-only remove; Status workflow-only; Issue exceptions-only
