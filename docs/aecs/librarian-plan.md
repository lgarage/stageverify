...
**Learning loop (worker → future packet):** Hard-earned fixes ship via `npm run away:ship -- --learned "…"` (or `--failure` + `--fix`); capture runs inline to `indexer-memory.json` — no manual `indexer:ingest`. `--note` auto-parses `root cause:`, `fix:`, `prod verify fail`, `stale gh-pages`. **`verify:*` failures** auto-queue to `learning-pending.json` via `run-verify-with-learning.mjs` wrapper; `away:ship` merges pending into indexer-memory. `npm run away:validate` runs `indexer:demo-packet` + `indexer:demo-verify-failure` regressions. Future work: `npm run away:next -- --packet` injects `gateWarnings` + lessons § + top-2 indexer matches. Promote high-signal gates to `gotcha-map.json` via `--apply-gotcha` when ready.
