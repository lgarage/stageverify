...
**Catch-all view-mode tile + top-bar badge (2026-07-24, v0.0.140)** — Staging Map shows persisted catch-all overlay in view after Done (read-only). **v0.0.304:** top-bar badge + CA map spot + status drawer count unique active deliveries assigned to Catch-all (`countCatchAllAssignedDeliveries`, same occupancy exclusions). `catchAllPendingCheckInCount` remains CF notify/check-in increment only — not the map count. Verify: `verify:catch-all-map` + `npx tsx scripts/test-catch-all-assigned-count.mjs` + `node scripts/verify-catch-all-count.mjs`.
