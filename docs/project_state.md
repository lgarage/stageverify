...
**Vendor login list paint (2026-08-25, v0.0.327–328)** — Company-PIN Submit paints `{VENDOR} DELIVERIES` immediately (PinGate no longer awaits settings; list fetch on PIN success; `#/s` eager). List-DTO cards paint before `getVendorReceiveDetails` (after-paint, concurrency 3). Live `index-CIiJqL62.js`. Chromium cold last-digit→first card 1952ms. Real iPhone 12–13s still unproven. No CF/DTO/PIN change. Verify: `verify:vendor-run-job-actions`.
