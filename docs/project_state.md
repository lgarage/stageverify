...
**Drawer Order Summary unified list (2026-08-23, v0.0.307)** — Delivery Details Order Summary shows every line in one table: unfinished first, delivered last (green `Delivered`). Separate Items Received accordion removed. `issueRows`/`receivedItems` banner semantics unchanged. Verify: `verify:delivery-consistency` + `verify:delivery-drawer-status`.
