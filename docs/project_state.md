...
**Active phase:** **MVP done — 100.00%** (`MVP_PATH.md`; SSOT reconciled 2026-07-16). §14 E2E residual closed (away-130, 2026-07-17). Live email band closed (away-128/129). **Location-first Phase 4** ✅ (`v0.0.44+`). Location-first Phases 5–6 deferred post-MVP.
...
**Vendor PIN gate (shipped 2026-06-08):** 4-digit PIN keypad after QR scan on `/#/receive`. `verifyVendorPin` CF validates PIN against order?s vendor; `vendors` collection auth-only read; 15-minute session timeout; audit log in `pinVerificationEvents`. Demo: `vendor-1` / `delivery-demo-vendor-1` no longer ships a hardcoded seed PIN — use the configured company or job PIN.
...
**Dispatcher Deliveries page size (2026-08-25, v0.0.330)** — Deliveries table Per page selector 25 / 50 / 100 / All. All shows the current filtered/sorted client-side result set (same `listDeliveries` fetchAll + filter/sort; no extra query). Default 25. Verify: `test:delivery-list-paging` + `verify:dispatcher-deliveries-page-size`.

**Vendor login CF path (2026-08-24, v0.0.325)** — `getVendorRunDeliveries` batches independent job/PO/location/item reads; `resolveLocationScanPin` parallelizes technician/job/vendor/config/location lookups after sequential rate-limit writes. Job PIN still wins when the same digits match another vendor’s company PIN. Response DTO unchanged. Deploy only those two functions. Verify: `test:resolve-location-scan-pin`, `test:get-vendor-run-deliveries`.
**Vendor login CF minInstances (2026-08-28)** — `resolveLocationScanPin` + `getVendorRunDeliveries` `onCall` `minInstances: 1` LIVE (PR #231 / `28d9b249`; hashes `a2827d60…` / `7ee19227…`). No body/DTO/PIN/session/rules/FE change. Post-deploy unused T0→T4 still 3.6–5.4s; warm CF path 2.34s. Verify: `test:resolve-location-scan-pin`, `test:get-vendor-run-deliveries`, prod `#/s?loc=G1&svdebug=1`.
**Vendor login CF body opt (2026-08-28)** — Same two functions only: hoist `resolveManagementPinMatch` into the first lookup wave; parallel vendor audit+session writes (rate-limit clear still after); list items via chunked `in` (no query-level limit; cap 50/delivery in memory). PIN/session/DTO/rules/FE unchanged. LIVE merge `a08cadf7` / PR #232; hashes `571760d9…` / `37f06696…`. Prod unused T0→T4 1240ms then 4930ms; warm 1520ms. List 220–300ms when up. Unused not reliable &lt;2s. Verify: `test:resolve-location-scan-pin`, `test:get-vendor-run-deliveries`.

**Email Vendor item display (2026-08-24, v0.0.326)** — Email Vendor outstanding-item bullets reuse `getVendorItemDisplay` (presentation-only). Stored `items.description` / parser truth unchanged. PR #195 reason selection unchanged. Verify: `scripts/test-vendor-comms-issue-draft.mjs` + `scripts/test-vendor-item-display.mjs`.
**Email Vendor draft reason selection (2026-08-24, v0.0.324)** — Dispatcher Email Vendor subject/body uses structured reason priority (blocking material issue → outstanding/backorder/partial → true missing staging → follow-up). Stops copying WNA "Staging location missing" via `/missing/` string match when required items are still outstanding. PR #192 `deliveryHasCurrentShopStagingAssignment` and WNA compact copy unchanged. Verify: `scripts/test-vendor-comms-issue-draft.mjs`.
**Vendor mobile active/completed visibility (2026-08-23, v0.0.323)** — Company-PIN list only: Partial → open → fully completed `<24h`; collapsed **Completed deliveries** for 24–72h (`<=72h` inclusive); `>72h` vendor-hidden. Age uses `vendorPhysicalDropoffConfirmedAt` from Complete (`applyVendorDeliveredReceiving`); Undo clears it. Visibility filter only — no delete/archive. Job-PIN `orderVendorJobsDeliveredLast` unchanged. Verify: `verify:vendor-run-job-actions` + `scripts/test-vendor-job-list-order.mjs`.
**Vendor mobile Partial/backorder cards (2026-08-23, v0.0.313)** — Company-run job cards show authoritative order fulfillment (Partial / Delivered) separately from physical drop-off. Partial+backorder cards use PARTIAL chrome; expanded lines show BACKORDERED. Completed physical drop-off copy is **Physical drop-off complete** / **Undo drop-off**. PR #177 per-card actions and PR #173 physical delivered-last sort unchanged. FE hydrate via `getVendorReceiveDetails` when list DTO lacks qty. Verify: `verify:vendor-run-job-actions` + `scripts/test-vendor-job-card-status.mjs` + `scripts/test-vendor-job-list-order.mjs`.

**Drawer Order Summary unified list (2026-08-23, v0.0.308)** — Delivery Details Order Summary shows every line in one table: unfinished first, delivered last (green `Delivered`). Separate Items Received accordion removed. `issueRows`/`receivedItems` banner semantics unchanged. Verify: `verify:delivery-consistency` + `verify:delivery-drawer-status`.
...
**Catch-all view-mode tile + top-bar badge (2026-07-24, v0.0.140)** — Staging Map shows persisted catch-all overlay in view after Done (read-only). **v0.0.304 LIVE:** top-bar badge + CA map spot + status drawer count unique active deliveries assigned to Catch-all (`countCatchAllAssignedDeliveries`, same occupancy exclusions). **v0.0.361:** top-bar **Catch-all delivery** tap opens a compact quick sheet of those same assigned deliveries (`listCatchAllAssignedDeliveries`; count = list.length). Button/badge chrome unchanged; notify stays a secondary sheet action. **v0.0.362:** Delivery Details **Close** (and Esc/overlay) strips `openDelivery` from the hash before clearing drawer selection so Catch-all deep-links do not immediately re-open the drawer on phone Safari. **Dan iPhone ACCEPT 2026-08-29** (sheet + Close on 6169306 / Twin Pillar). `catchAllPendingCheckInCount` remains CF notify/check-in increment only — not the map count. Verify: `npx tsx scripts/test-catch-all-assigned-count.mjs` + `node scripts/verify-catch-all-quick-sheet.mjs` + `verify:catch-all-map` + `node scripts/verify-catch-all-count.mjs`.
...
**Drawer email review CTA (2026-07-03 / v0.0.334)** ? When vendor email proposal needs dispatcher review (e.g. ORD-006), **What Needs Attention** shows primary **Review Vendor Email** button. v0.0.334: invoice-source modal shows stored From/Date/Subject + honest empty body (not PDF OCR). Verify: `verify:review-vendor-email-source` + `verify:delivery-drawer-status`.
