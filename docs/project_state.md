...
**Exception-only vendor flow (shipped 2026-06-11; delivered-card collapse v0.0.258; vendor-run unify v0.0.261):** Delivered hub with Need More Space, Report a Problem, and `markVendorDelivered` (status `arrived`, not `ready_for_pickup`). After Mark Delivered, that card auto-collapses in place to a compact green DELIVERED row (independent expand/collapse; Undo when expanded; vendor-run list keeps stable order). Company-wide vendor-run list uses the same hub shell/card chrome, card gaps, safe-area Back above Delivered, and Undo on expanded delivered rows. E2E: `npm run verify:vendor-delivered`. Monday-safe: `node scripts/verify-vendor-monday-safe.mjs` (Mark Delivered disabled without assignable spot + collapse/gap/Back/undo asserts).
