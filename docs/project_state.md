...
**Vendor login CF minInstances (2026-08-28)** — `resolveLocationScanPin` + `getVendorRunDeliveries` `onCall` `minInstances: 1` (us-central1, 256MB, nodejs22). No body/DTO/PIN/session/rules/FE change. Deploy only those two functions. Verify: same two test scripts + unused vs warm prod `#/s?loc=G1&svdebug=1`.
