...
Phase 2 gate passed 2026-06-08. **Phase 4 gate closed 2026-06-20** — pickup issue resolution readback + `verify:phase4-integration`. **Phase 3 gate closed 2026-06-19–20** (integration verify + permanent shop-stock mapping). **Batch 3 shipped 2026-06-17–18:** vendor session (Slice 4), pickup tokens (Slice 5), §10–§11 pickup UI, combination staging stub, Phase 4 issue resolve (`away-021`…`041`; see `archives/away-batch-3.json`). **Away batch 047–053 (2026-06-19):** pickup submit label, Phase 5 email panel polish, public network-error UX, resolution-type picker, Phase 4 integration verify, **Phase 5 inbox settings UI** (`away-053`). **Away-054..057 (2026-06-21):** fingerprint dedup, `verify:phase5-email`, correction-to-earlier-email fixture, expanded proposal review detail panel. **Away-058 (in progress):** delivery-drawer READINESS EVIDENCE (Condition 1 offline proposals scoped per delivery, Condition 2 physical fields, blockers — read-only). Future email parent-match strategy (Job rollup vs PO rollup) deferred to Phase 6+ Settings — not implemented in Phase 5.
