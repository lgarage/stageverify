...
**Vendor login CF body opt (2026-08-28)** — Same two functions only: hoist `resolveManagementPinMatch` into the first lookup wave; parallel vendor audit+session writes (rate-limit clear still after); list items via chunked `in` (no query-level limit; cap 50/delivery in memory). PIN/session/DTO/rules/FE unchanged. Verify: `test:resolve-location-scan-pin`, `test:get-vendor-run-deliveries`. Prod unused &lt;2s still pending re-measure + Dan iPhone.
