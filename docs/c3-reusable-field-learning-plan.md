...
**Status:** Architecture handoff captured 2026-08-09 from the C3 design conversation (Composer + Grok Critical/Q&A).  
...
| Phase | Status |
| ----- | ------ |
| **C3-A** | **Handoff written; D-59 #7/#8 amend text drafted below — PENDING APPROVAL / NOT YET AUTHORITATIVE in `DECISIONS.md`** |
| **C3-B** | not started |
| **C3-C** | not started |
| **C3-D** | not started |
| **C3-E** | not started |
...
**NEXT C3 JOB:** **C3-A** — land Dan-approved D-59 #7/#8 amendment in `PROJECT_STATUS/DECISIONS.md` (+ training-note-ignore-spec addendum sync). Do **not** start C3-B product parser work until C3-A is authoritative, unless Dan explicitly queues parser harden independently.
...
**PENDING APPROVAL / NOT YET AUTHORITATIVE.**

As of this handoff, `PROJECT_STATUS/DECISIONS.md` **D-59** still records:

- **(7)** structured-lesson MVP category = **fulfillment-method mapping only**
- **(8)** structured interpretation engine **deferred** — playbook prose stays; fulfillment mapping is the first category when P8 gets its own spec

Lane C addendum on `docs/training-note-ignore-spec.md` states C3 requires amending #7/#8 but **does not itself amend** the decision prose.

Until Dan approves and an agent lands the amend in `DECISIONS.md` (phase **C3-A**), the draft below is **architecture intent only** — not an authoritative decision.

### Draft amend text (C3-A target — land only after Dan approval)

> **(7) amended:** Structured reusable-lesson categories are allowlisted and fail-closed. Allowed categories:
>
> 1. `fulfillment_mapping` — deferred to P8 (unchanged intent).
> 2. `header_field_extraction` — Lane C C3 only; may cover **only**:
>    - `customerPoOrReference`
>    - `vendorOrderNumber`
>    - `vendorInvoiceNumber`
>
> Explicitly lifts the prior “field mapping beyond fulfillment” ban **solely** for those three fields. Still rejected: document classification overrides, normalization mapping, review-routing rules, **parser confidence adjustment as a rule effect**, vendor-layout / semantic fingerprint rules (v1), cross-vendor wildcards, auto-approve, delivery/job/item mutation, and any non-enumerated category.
>
> **(8) amended:** A full general-purpose structured interpretation engine remains deferred (P8 for fulfillment). **Carve-out:** C3 may ship a **deterministic** promotion path — verified C2 audits → candidate examples → proposed lesson → Manager/Admin activation → active extraction overlay — **without** waiting for the P8 model→schema engine and **without** adding a free-form note→LLM interpreter for field lessons. Any future model-interpreted lesson path requires its own Fable war-game + approval.
...
- **“3 examples” is a product policy threshold, not mathematically proven confidence.** It is the recommended default from the design conversation; Dan may lock a different number at C3-A/C3-D approval.
- **`dispatcher_assertion`-only examples must not satisfy the threshold by themselves.** Promotion requires a majority (recommended: all counting examples) of `document_evidence`.
...
| **Completion status** | **Pending** — draft in §4; not in `DECISIONS.md` yet |
...
| Phase | Status |
| ----- | ------ |
| C3-A | Handoff + draft amend ready; **PENDING APPROVAL / NOT YET in DECISIONS.md** |
| C3-B | not started |
| C3-C | not started |
| C3-D | not started |
| C3-E | not started |
...
**NEXT C3 JOB:** **C3-A** — obtain Dan approval and land the D-59 #7/#8 amendment (+ sync training-note-ignore-spec addendum). Then proceed to **C3-B** (Johnstone universal miss harden from real corpus) before any parse-effect lesson work.
...
- Start C3-C/D/E while D-59 #7/#8 remain unamended in `DECISIONS.md`
...
*End of C3 reusable field learning plan. Update the Status table when each phase ships; keep amend wording in sync with `DECISIONS.md` after C3-A lands.*
