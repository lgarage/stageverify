...
- Last shipped: **v0.0.232** — merge PR #46 technician assignment UX: Settings release checklist removed; drawer-only ASSIGNED TECHNICIAN / exclusive single-tech assign; gh-pages LIVE; `verify:settings-technicians:prod` + `verify:dispatcher-job-release:prod` + `verify:admin-appearance` PASS. [fast-safe UI]
