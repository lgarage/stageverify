...
- Last shipped: **v0.0.82** — Staging Map edit: ground spots in fixed absolute slots (sibling size unchanged on move), `mapWidth`/`mapHeight` resize with panel + corner handle, persist on save / revert on cancel.
...
## Active Blockers
1. **Shelving decision (Jake Korb)** — layout IDs provisional (Jake concept locked for v1 map).
