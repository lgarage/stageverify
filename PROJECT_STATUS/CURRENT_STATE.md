...
- Last shipped LIVE: **v0.0.249** — Vendor Drop-Off **Phase 1 performance** (PR #74): CF `verifyVendorPin` bootstrap + gh-pages PIN UX/hub progressive paint + keypad visual polish; no `minInstances`. Merge `f8d86184`. [high-risk CF + fast-safe FE]
...
## Immediate Next Step
- **Vendor Drop-Off Phase 1** merged (`f8d86184`) — deploy CF `verifyVendorPin` + gh-pages **v0.0.249**, then production warm PIN→useful hub re-measure (target ≤1500 ms). Do **not** enable `minInstances` until cold evidence reviewed.
