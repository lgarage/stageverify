...
- Last shipped LIVE: **v0.0.243** — portal sidebar full `stageverify-logo-main.svg` wordmark in white frame via shared `PortalSidebar` / `StageVerifyBrandMark` (`variant="wordmark"`); no duplicate STAGEVERIFY text; Dispatcher Portal sublabel kept; print chrome unchanged. Merge PR #63; gh-pages only (no CF/rules). [fast-safe UI deploy]
