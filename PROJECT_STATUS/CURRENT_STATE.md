...
- **In flight (PR only, no deploy):** Lane C **C1** Invoice Review Chat — `reviewAgentTurn` CF + `vendorInvoiceImportChats` rules + inspect-modal chat UI (v0.0.246). Training note collapsed under **Advanced** (chat primary). In-modal multi-turn verified via mock+sessionStorage (prod schema); live CF/rules still post-merge. Before C3: amend D-59 #7/#8 for field learning.
- **Merged on main (awaiting/confirm deploy separately):** PR #64 invoice Parsed Import **staging-on-Approve** + **D-67 matched-delivery** — CF Approve targets high-confidence matched existing delivery (no competing shell) or shell `delivery-vii-{importId}`; Drop-Off writes/preserves `plannedStagingLocationIds`; Will-Call skips. Merge `7161e271`. [high-risk CF + fast-safe UI]
- Last shipped LIVE: **v0.0.244** — merge PR #57 Vendor `companyName`/`locationName` + Company — Location display helper + larger Vendor Communications workspace (optional delivery, email auto-populate); **gh-pages only** (no CF/rules/migration). Merge `ab21e246`; Pages tip `54fe0949`. Prod `verify:dispatcher-nav:prod` PASS. [fast-safe UI deploy]
...
## Immediate Next Step
- Dan/ops: Settings → **Reconnect Gmail**, then smoke one invoice through ingest → Inbox archive + View Original PDF.
