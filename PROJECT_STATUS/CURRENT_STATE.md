...
- **Partial deploy:** D-59 P2–P7 on `main` (`49924c8b`, v0.0.204). **gh-pages LIVE** @ **v0.0.240** (row-scoped Admin Access UI from PR #56; merge tip after PR #58 `8b449d4c`). **Firebase rules** for accessPinSecrets already deployed (Phase 1). **CF:** PIN/Admin Access + dual-read verify paths LIVE (no redeploy after PR #58 — live lib hashes matched tip). Sticky-reject / Gmail auto-archive prior deploys unchanged.
...
- Last shipped LIVE: **v0.0.240** — Settings **row-scoped Admin Access** gh-pages only (PR #56 UI); bundle `index-DMMfCUfs.js`; PR #58 dual-read CF already live (no redeploy); PIN length exactly 4 FE+BE (≤6 STOP clear). Prod Admin Access checklist PASS + tech/vendor/management PIN login PASS. [fast-safe UI deploy]
