...
- **In flight (PR only, no deploy):** invoice Parsed Import **staging-on-Approve** + **D-67 matched-delivery** — CF Approve targets high-confidence matched existing delivery (no competing shell) or shell `delivery-vii-{importId}`; Drop-Off writes/preserves `plannedStagingLocationIds`; Will-Call skips. Branch `cursor/invoice-review-staging-approve-bad0` (PR #64). Do not merge until Dan confirms. [high-risk CF + fast-safe UI]
