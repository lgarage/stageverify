...
- **Partial deploy:** D-59 P2–P7 on `main` (`49924c8b`, v0.0.204). **gh-pages LIVE** @ v0.0.237. **Firebase rules NOT deployed** — Sonnet pre-deploy APPROVE (`bf2570ff…`); console TTL on `trainingNoteAudit.expireAt` still needed after rules deploy. **CF deployed:** credit-return delivery block (ingest auto-reject + approve/create_shell/relink guard) @ v0.0.217; prior `approveVendorInvoiceImport` credit-return reject @ v0.0.215 (`eb000e7`); `recalculateDeliveryReadiness` + will-call preserve @ v0.0.214 (`5f1f575`); `recordPickupEvent` (`4755802`); **reject-preserve Gmail sync** @ v0.0.213 (`9529530`).
- **In flight (PR only, hold deploy):** Monday-MVP dispatcher usability — staging Location column parse/`locById` fix (`id: docId`); global text selection restored; drawer fulfillment **Will-Call / Pickup from Vendor**; list empty/will-call staging shows **—**. Branch `cursor/monday-mvp-dispatcher-usability-8643`. [fast-safe UI + display helper]
- Last shipped: **v0.0.237** — merge PR #54 Monday-MVP dispatcher usability: staging Location column parse/`locById` (`id: docId`); global text selection restored; drawer **Will-Call / Pickup from Vendor**; list empty/will-call staging **—**; gh-pages only (no CF/rules). Merge `2f3b4ec8`. [fast-safe UI]
...
## Immediate Next Step
- Dan/ops: hard-refresh Settings after v0.0.235 for unified PIN & Access Management; Exception-only Delivered hub when ready; vendor phone Location/Invoice from v0.0.227+.
