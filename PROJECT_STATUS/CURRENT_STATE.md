...
## Snapshot
- **Standing harness:** **D-47** conf ≥ 97% (no premium call solely to raise score — D-65); **D-60/D-38 PROTECTED** on high-risk (Sonnet 5 `claude-sonnet-5-thinking-high`); **D-65 Balanced trim** LIVE @ `9f5cc0b0` — risk-based lanes: simple UI→Composer; visual-judgment UI→Sol High Task; ≤1 Grok on routine T1; Ship skip tiny-fast-safe FE; `ship:evidence-preflight` before Ship; Q&A consequential-only; SSOT `ui-model-routing.json` v3. Evidence: critical `fad8da02` Agree; sonnet-instruct `24bdb9aa` Agree; sonnet-verify `4f245217` PASS (`gate:check:test` 63/63); security-gate `f4c75b8e` PASS; build-checker `1dc72579` PASS.
- **MVP: 100.00% — done** — SSOT reconciled 2026-07-16 (`MVP_PATH.md`). §14 E2E prod re-verify **PASS** away-130 (2026-07-17).
- **Partial deploy:** D-59 P2–P7 on `main` (`49924c8b`, v0.0.204). **gh-pages LIVE** @ **v0.0.239** (`e33607fa`, bundle `index-B_8HJesY.js`). **Firebase rules NOT deployed** — Sonnet pre-deploy APPROVE (`bf2570ff…`); console TTL on `trainingNoteAudit.expireAt` still needed after rules deploy. **CF deployed:** sticky-reject — `approveVendorInvoiceImport` + `bulkReopenImportsSkippedByRule` @ `e33607fa` (2026-08-08); Gmail auto-archive (D-64) — `syncInboundGmail` + `gmailInboxPushIngest` + `triggerInboundGmailSyncCallable` + `initiateGmailOAuth` + `completeGmailOAuth` @ merge `9b908abd`; prior credit-return delivery block @ v0.0.217; `recalculateDeliveryReadiness` @ v0.0.214; `recordPickupEvent` (`4755802`); reject-preserve Gmail sync @ v0.0.213.
...
- Last shipped LIVE: **v0.0.239** — sticky manual invoice reject LIVE (CF + gh-pages): CF blocks `reopen` unless `isSystemAutoRejectedImport`; UI hides Re-open for manual rejects; harness never clicks Re-open. Prod sticky verify PASS (6168733 + 3316448A stay rejected; callable reopen blocked). [high-risk CF + fast-safe UI/harness]
