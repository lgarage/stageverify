...
## Snapshot
- **Standing harness:** every session honors **D-47** conf ≥ 97% before any file edit; **D-60** high-risk Sonnet instruct→verify loop on auth/CF/rules ships (`high-risk-sonnet-loop.mdc`); **D-38** security gate = Sonnet 5 (`claude-sonnet-5-thinking-high`) — Sonnet 4.6 retired from active allowlist (2026-08-08); **D-63** UI/UX → Sol High primary (`gpt-5.6-sol-high`; Medium unavailable) + Grok verifier + readability DoD — SSOT `ui-model-routing.json`; non-UI Composer unchanged.
...
- Last shipped: **v0.0.230** — admin/dispatcher UI modernization (shared `.portal-shell` radius/spacing/control/shadow tokens + utilities; cards/tables/toolbars/drawers/modals); Light/Dark persistence unchanged; vendor/technician mobile unchanged; `verify:admin-appearance` + `verify:dispatcher-nav` PASS. [fast-safe UI]
