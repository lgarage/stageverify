...
- **Partial deploy:** D-59 P2–P7 on `main` (`49924c8b`, v0.0.204). **gh-pages LIVE** @ v0.0.214 (`ee696f3`). **Firebase rules NOT deployed** — Sonnet pre-deploy APPROVE (`bf2570ff…`); console TTL on `trainingNoteAudit.expireAt` still needed after rules deploy. **CF deployed:** `recalculateDeliveryReadiness` + `approveVendorInvoiceImport` @ v0.0.214 will-call preserve (`5f1f575`/`ee696f3`); prior `recordPickupEvent` (`4755802`); **reject-preserve Gmail sync** @ v0.0.213 (`9529530`).
- Last shipped: **v0.0.214** @ `ee696f3` — will-call Confirm Pickup stuck on board: invoice shell Gmail re-sync (`buildInvoiceShellPatchDocument`) was resetting `status`/`invoiceImportStatus` to `ready_for_pickup` after pickup; readiness recalc now preserves/repairs `closed_picked_up`; client validates CF `picked_up` before drawer close; prod repair script patched INV-6168732 + INV-6167990 to `picked_up`/`closed_picked_up`. [high-risk CF]
...
## Immediate Next Step
- Dan: hard-refresh dispatcher Deliveries board — INV-6168732 + INV-6167990 repaired to `picked_up`/`closed_picked_up`; should leave default board; use **Picked Up** filter to confirm.
