...
- **In flight (PR only, no deploy):** invoice Parsed Import **staging-on-Approve** — CF `approveVendorInvoiceImport` writes `plannedStagingLocationIds` for Vendor Drop-Off; Will-Call skips; picker SoT `useLiveZoneOccupancy`. Branch `cursor/invoice-review-staging-approve-bad0` (PR #64). [high-risk CF + fast-safe UI]
