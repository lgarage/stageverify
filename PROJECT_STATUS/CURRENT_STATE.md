...
- **In flight (PR #51):** dispatcher Deliveries table final column order — Status → Fulfillment → Vendor → Job Name → Invoice # → PO # → Staging Location → Items → Delivery / Pickup Date → Issue → Assigned Technician → Action; Job #/Order # table-only remove (drawer+search kept); Status workflow-only; Issue exceptions-only. Synced onto main tip (incl. PR #50 PIN unify + brand assets, package v0.0.235). [fast-safe UI] branch `cursor/dispatcher-deliveries-column-order-b366`.
...
- Prior: **main tip** — merge PR #50 Settings PIN & Access Management unify (Manager/Dispatcher Auth into PIN roster) + StageVerify brand SVG assets; package v0.0.235 on tip (gh-pages still reporting v0.0.234 until next deploy).
...
## Immediate Next Step
- Review/merge dispatcher Deliveries column-order PR #51 (`cursor/dispatcher-deliveries-column-order-b366`) after SAFE TO MERGE report; then Dan/ops: Exception-only Delivered hub when ready; vendor phone Location/Invoice from v0.0.227+.
