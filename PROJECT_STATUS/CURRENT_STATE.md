...
- Last shipped: **Firestore forward-only delivery status rules** deployed to `stageverify-db` (2026-06-12, commit `0556ae4`); prod verify: vendor PIN/demo + pickup PASS; unauth `picked_up→partial` denied on prod; no console permission errors on receive/pickup routes.
