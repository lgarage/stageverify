...
- **In flight (PR only — hold deploy):** StageVerify brand unification — authoritative `public/logos/stageverify-*.svg` (main `@3b00a70e`); shared `brandTokens` + `StageVerifyBrandMark`; sidebar/login/vendor+tech PIN/hub chrome + favicon; Sol High + Grok UI Playwright PASS; branch `cursor/stageverify-branding-pass-2d28`. [fast-safe UI]
