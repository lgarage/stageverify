...
- Last shipped: **Invoice approve → dashboard visibility + no silent shell failure** — review-only Approve auto-creates job from P/O when unmatched; CF returns `shellCreated`/`jobCreated`; backfill errors surface in Invoice Review + Refresh Now; success banner + portal refresh on approve
