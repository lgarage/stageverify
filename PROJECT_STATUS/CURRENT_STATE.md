...
- **Partial deploy:** D-59 P2–P7 on `main` (`49924c8b`, v0.0.204). **gh-pages LIVE** @ **v0.0.248**. **Lane C C1+C2 Invoice Review Chat LIVE** (`reviewAgentTurn` `reviewagentturn-00004-qok` + `applyInvoiceReviewFieldCorrection` `applyinvoicereviewfieldcorrection-00001-xod` + rules; merge `c0375110` / tip `b0bdf17b`, D-68; gh-pages bundle `index-BjAkuztY.js`). **Vendor company/location + Vendor Communications workspace LIVE** (PR #57). **Access PIN migration Phase 1 + Phase 2 COMPLETE**; **Admin Access UI LIVE**; **PIN length 4–6 LIVE**; **inactive Manager/Dispatcher Remove LIVE** (`removeDispatcher`). Smoke PR50 Auth accounts removed from roster. Sticky-reject / Gmail auto-archive prior deploys unchanged.
