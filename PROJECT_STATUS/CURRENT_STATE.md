...
- **In flight (PR #51):** dispatcher Deliveries table final column order — Status → Fulfillment → Vendor → Job Name → Invoice # → PO # → Staging Location → Items → Delivery / Pickup Date → Issue → Assigned Technician → Action; Job #/Order # table-only remove (drawer+search kept); Status workflow-only; Issue exceptions-only. Synced onto main @ v0.0.234. [fast-safe UI] branch `cursor/dispatcher-deliveries-column-order-b366`.
