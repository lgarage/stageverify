...
- **WIP (branch `cursor/technician-assignment-ux-61d0`, PR only — not merged/deployed):** Settings PIN & Access — removed "Release jobs for today" block; dispatcher drawer job panel copy → ASSIGNED TECHNICIAN / Assign Technician; **single-tech assignment** (one job = one technician; exclusive assign strips stale multi-holders).
