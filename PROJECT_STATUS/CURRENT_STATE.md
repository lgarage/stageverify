...
- In flight: **Gmail auto-archive after durable ingest (D-64)** — branch `cursor/gmail-auto-archive-75ba`; remove INBOX only after `parsed`/`reply_processed`; View Original PDF stays Gmail fetch; durable PDF = away-138 (blocked). [high-risk CF + OAuth `gmail.modify` — reconnect after deploy]
