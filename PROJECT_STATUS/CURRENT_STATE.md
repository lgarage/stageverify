...
- **Partial deploy:** D-59 P2–P7 on `main` (`49924c8b`, v0.0.204). **gh-pages LIVE** @ v0.0.214 (`ee696f3`). **Firebase rules NOT deployed** — Sonnet pre-deploy APPROVE (`bf2570ff…`); console TTL on `trainingNoteAudit.expireAt` still needed after rules deploy. **CF deployed:** `recalculateDeliveryReadiness` + `approveVendorInvoiceImport` @ v0.0.214 will-call preserve (`5f1f575`); prior `recordPickupEvent` (`4755802`); **reject-preserve Gmail sync** @ v0.0.213 (`9529530`).
- Last shipped: **v0.0.214** @ `ee696f3` — systemic will-call pickup preserve: Gmail invoice shell re-sync no longer overwrites terminal `picked_up`/`closed_picked_up`; readiness recalc preserves/repairs terminal pickup; client validates CF `picked_up` before drawer close. [high-risk CF]
...
## Immediate Next Step
- Dan: hard-refresh Deliveries board; for any will-call still showing Will-Call / Pickup after a prior Confirm Pickup, open drawer and Confirm Pickup again — systemic fix prevents future Gmail sync from reverting; jobs picked up after deploy leave default board automatically.
