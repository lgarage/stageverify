...
## Snapshot
- **Standing harness:** **D-47** conf ≥ 97% (no premium call solely to raise score — D-65); **D-60/D-38 PROTECTED** on high-risk (Sonnet 5 `claude-sonnet-5-thinking-high`); **D-65 Balanced trim** — risk-based lanes: simple UI→Composer; visual-judgment UI→Sol High Task; ≤1 Grok on routine T1; Ship skip tiny-fast-safe FE; `ship:evidence-preflight` before Ship; Q&A consequential-only; SSOT `ui-model-routing.json` v3.
- **MVP: 100.00% — done** — SSOT reconciled 2026-07-16 (`MVP_PATH.md`). §14 E2E prod re-verify **PASS** away-130 (2026-07-17).
- **Partial deploy:** D-59 P2–P7 on `main` (`49924c8b`, v0.0.204). **gh-pages LIVE** @ v0.0.238. **v0.0.239 sticky-reject on main — CF + gh-pages DEPLOY HELD** pending Dan SAFE TO DEPLOY. **Firebase rules NOT deployed** — Sonnet pre-deploy APPROVE (`bf2570ff…`); console TTL on `trainingNoteAudit.expireAt` still needed after rules deploy. **CF deployed:** Gmail auto-archive (D-64) — `syncInboundGmail` + `gmailInboxPushIngest` + `triggerInboundGmailSyncCallable` + `initiateGmailOAuth` + `completeGmailOAuth` @ merge `9b908abd`; prior credit-return delivery block @ v0.0.217; `approveVendorInvoiceImport` @ v0.0.215; `recalculateDeliveryReadiness` @ v0.0.214; `recordPickupEvent` (`4755802`); reject-preserve Gmail sync @ v0.0.213.
...
- Last shipped (code on main, not LIVE): **v0.0.239** — sticky manual invoice reject: CF blocks `reopen` unless `isSystemAutoRejectedImport`; UI hides Re-open for manual rejects; verify-invoice-review never clicks Re-open on live Firebase. [high-risk CF + fast-safe UI/harness] **Deploy HELD.**
- Prior LIVE: **v0.0.238** — dark-mode navy layered surfaces (L0–L3); merge PR #53 `4b406c13`. [fast-safe UI]
