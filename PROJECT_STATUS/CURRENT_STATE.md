...
- **In flight (PR only — hold deploy):** StageVerify brand unification (PR #52) — synced onto `main` tip v0.0.236; authoritative `public/logos/stageverify-*.svg` (`@3b00a70e`); shared `brandTokens` + `StageVerifyBrandMark`; sidebar/login/vendor+tech PIN/hub + favicon; preserve PIN Access + Deliveries column-order from main. [fast-safe UI]
- Last shipped: **v0.0.236** — merge PR #51 dispatcher Deliveries final column order (Status → Fulfillment → Vendor → Job Name → Invoice # → PO # → Staging Location → Items → Delivery / Pickup Date → Issue → Assigned Technician → Action); Job #/Order # table-only remove; Status workflow-only; Issue exceptions-only; gh-pages only (no CF/rules). Merge `b183b4e6`. [fast-safe UI]
