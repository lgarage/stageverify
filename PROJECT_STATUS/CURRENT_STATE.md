...
- **Partial deploy:** D-59 P2–P7 on `main` (`49924c8b`, v0.0.204). **gh-pages LIVE** @ v0.0.235. **Firebase rules NOT deployed** — Sonnet pre-deploy APPROVE (`bf2570ff…`); console TTL on `trainingNoteAudit.expireAt` still needed after rules deploy. **CF deployed:** credit-return delivery block (ingest auto-reject + approve/create_shell/relink guard) @ v0.0.217; prior `approveVendorInvoiceImport` credit-return reject @ v0.0.215 (`eb000e7`); `recalculateDeliveryReadiness` + will-call preserve @ v0.0.214 (`5f1f575`); `recordPickupEvent` (`4755802`); **reject-preserve Gmail sync** @ v0.0.213 (`9529530`).
- **In flight (PR #51):** dispatcher Deliveries table final column order — Status → Fulfillment → Vendor → Job Name → Invoice # → PO # → Staging Location → Items → Delivery / Pickup Date → Issue → Assigned Technician → Action; Job #/Order # table-only remove (drawer+search kept); Status workflow-only; Issue exceptions-only. Synced onto main @ v0.0.235. [fast-safe UI] branch `cursor/dispatcher-deliveries-column-order-b366`.
- Last shipped: **v0.0.235** — merge PR #50 Settings **PIN & Access Management** unified roster (Manager/Dispatcher Auth + Technician/Vendor/Management PIN); separate Dispatcher accounts section removed; existing Auth/PIN stores unchanged; gh-pages only (no CF/rules). Prod Manager verify PASS (`verify:settings-technicians:prod` + `verify:settings-management-pins:prod` + `verify:settings-dispatchers:prod` + `verify:admin-appearance` @ prod). Merge `f9278fcb`. [fast-safe UI] Evidence: solution-verifier `bc-e290ef79` PASS; ui-implementer `bc-04688812` (`gpt-5.6-sol-high`, `fallback-from: gpt-5.6-sol-medium`); ui-readability PASS; ui-before-after settings-pin-access; ui-playwright-verifier `bc-91621592` PASS; build-checker `bc-1a279798` PASS; pre-deploy ship-verifier `bc-23bb4db8` PASS.
...
## Immediate Next Step
- Review/merge dispatcher Deliveries column-order PR #51 (`cursor/dispatcher-deliveries-column-order-b366`) after SAFE TO MERGE report; Dan/ops: hard-refresh Settings after v0.0.235 for unified PIN & Access Management; Exception-only Delivered hub when ready; vendor phone Location/Invoice from v0.0.227+.
