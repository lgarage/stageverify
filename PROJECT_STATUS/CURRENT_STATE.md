...
- Last shipped: **v0.0.222** (branch) — Monday-safe vendor hub: disable Mark Delivered when no assignable spot (“Ask dispatch for a staging spot.”); “Issue” → “Report a Problem”; receive path still Settings-gated `exception_only` (live config remains `full_checkin` until ops flip). [fast-safe UI — PR only, no deploy]
- Prior: **v0.0.219** — fix delivery drawer Reject gate for approved credit/return slip-throughs: Firestore hydration (same as list badge), nested-header + string-qty credit detection, `test:delivery-credit-reject-gate` + `verify:delivery-drawer-status` PASS. [fast-safe UI]
