...
11. **Approve identity + Deliveries visibility:** Approve-layer header identity is OR (`vendorOrderNumber` OR `customerPo`) — not S/O-only. Match-link Approve skips `create_shell`; prod `hideSeedDemoRows` needs `vendorInvoiceImportId` on the delivery — historical linked rows need Refresh Now / create_shell stamp. Match UI: humanize reasons, hide scores, navy selection (not brand RED), Optional Delivery ID labeled optional. Gotchas: `invoice-import`, `prod-verify-hide-seed-demo`.
