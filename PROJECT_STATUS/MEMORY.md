...
| Consistency check | `npm run away:validate` · `npm run away:sync` (`--write` after hand edits; `away:sync-next` = NEXT.md only) | Before commit; after memory edits; clears stale `instructions` when `sequence` empty |
