...
D-13 (2026-07-09) [harness]: Autonomy-evidence pairing — no new autonomy without evidence produced for a non-author; evidence means report lines/logs/verdicts, not infrastructure (dashboards must justify separately) — because authority without independent observation is how claimed-but-never-run reviews happened.
D-14 (2026-07-09) [harness]: Organizational concepts must delete existing complexity in the same commit or be rejected; capability additions instead face the 80/20 burden-of-proof (rejected when rare-case value < permanent complexity tax; catastrophic-asymmetry gates like the security gate are exempt) and must name a maintenance owner — because concepts that delete nothing become permanent layers.
D-15 (2026-07-09) [harness]: Harness V1 Freeze — asymmetric (deletions always legal; additions need a named dated pain event ≥2×, a security/production incident, a measured bottleneck, or a real customer gap); charter: PROJECT_STATUS/HARNESS_V1_FREEZE.md — because further harness work now delays StageVerify MVP (CTO decision, adversarially reviewed rounds 1-5).
